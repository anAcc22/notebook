...
/*
Range update range query segtree beats over arbitrary state
We note that segtree beats is a generalization of lazyprop segtree, but
you can customize when it stops pushing state down and when it puts a lazy tag
*/
...
struct Tree {
    ...
    void _upd(ll i, ll l, ll r, ll ql, ll qr, const U& updfn) {
        if(U::break_condition(ql, qr, l, r, v[i], updfn)) return;
        if(U::tag_condition(ql, qr, l, r, v[i], updfn)) {
            lazy[i] = lazy[i] + updfn;
    ...
    void _pupd(ll i, ll l, ll r, ll qi, const T& nv) {
        if(l == r) {
            v[i] = nv;
            return;
        }
        push(i, l, r);
        ll m = (l + r) >> 1ll;
        if(qi <= m) _pupd(i << 1ll, l, m, qi, nv);
        else _pupd((i << 1ll) | 1ll, m + 1ll, r, qi, nv);
        v[i] = v[i << 1ll] + v[(i << 1ll) | 1ll];
    }
    void pupd(ll qi, const T& nv) {
        _pupd(1ll, 0ll, n - 1ll, qi, nv);
    }
};
...
struct Mono {
    ll v;
    ll cmax;
    Mono(ll a_v, ll a_max): v(a_v), cmax(a_max) {};
    inline Mono operator+(const Mono& o) const {
        return Mono(v + o.v, max(cmax, o.cmax));
    }
    static inline Mono identity() {
        return Mono(0ll, NINF(ll));
    }
};
...
break_condition() tells the segtree when to stop propagating updates downwards
tag_condition() tells the segtree when to stop propagating updates downwards AND
mark the current node with a lazy update
! Mono must be a Monoid. That is, it must satisfy:
! (1) Associativity (e.g., (a+b)+c = a+(b+c) for addition)
! (2) Existence of an Identity Element (e.g., 0 + x = x for addition), and
! (3) Closure (e.g., a+b returns another integer if both a,b are integers)
...
struct Upd {
    ...
    Mono upd(ll l, ll r, const Mono& old) const {
        return l == r ? Mono(old.v % x, old.cmax % x) : Mono(old.v, old.cmax);
    }
    ...
    static inline bool break_condition(ll ql, ll qr, ll l, ll r, const Mono& state, const Upd& upd) {
        return ql > r || qr < l || upd.x > state.cmax;
    }
    static inline bool tag_condition(ll ql, ll qr, ll l, ll r, const Mono& state, const Upd& upd) {
        return ql <= l && l == r && r <= qr;
    }
};